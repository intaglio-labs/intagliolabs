...
test('readStore reads the AddressBook shape and skips nameless records', () => {
  ...
  // personRef is a content-derived hash (contacts.mjs's private personRefFor);
  // assert the fields this test owns and that the ref is a stable-looking
  // string, rather than pinning its value.
  assert.deepEqual(
    sorted.map(({ personRef, ...rest }) => rest),
    [
  ...
  for (const entry of sorted) assert.equal(typeof entry.personRef, 'string');
  assert.notEqual(sorted[0].personRef, sorted[1].personRef, 'distinct people get distinct refs');
});
