#!/usr/bin/env node
// Run one public-lookup pass (L5 step 6), spawned by the Distiller's timer
// (Swift-side, see widget/src/Distiller.swift) or by hand.
//
//   node ui/scripts/lookup-once.mjs --power trickle --battery 62 --on-ac 0 --thermal nominal
//   node ui/scripts/lookup-once.mjs --power full --limit 4
//
...
// a lookup pass WRITES pending claims about real people, so it goes through
// hermes' own bearer-only route (POST /admin/relationship/lookup) rather than
// a second process touching the file directly.
//
// NO PERSON KEYS OR TEXT ARE EVER PRINTED. person_lookup_run rows (what this
...
const KNOWN_FLAGS = new Set(['--limit', '--power', '--battery', '--on-ac', '--thermal']);
...
async function main() {
  ...
  let res;
  try {
    res = await fetch(`${hermesBase}/admin/relationship/lookup`, {
      method: 'POST',
