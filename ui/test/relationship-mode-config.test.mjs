...
test('the cap is written when the owner has none', () => {
  const configPath = tempConfig({ selfName: 'Owner', relationshipMemory: { mode: 'any' } });
  assert.deepEqual(ensureRelationshipCap({ capPerDay: 1, configPath }), { capPerDay: 1, changed: true });
  const written = read(configPath);
...
test('the cap already in the file is never overwritten', () => {
  // THE WHOLE POINT OF "ensure". A second run of onboarding -- a reinstall, a
  // replayed flow -- must not walk an owner's 5 back down to 1.
  const configPath = tempConfig({ relationshipMemory: { capPerDay: 5 } });
  const before = statSync(configPath).mtimeMs;
  assert.deepEqual(ensureRelationshipCap({ capPerDay: 1, configPath }), { capPerDay: 5, changed: false });
  assert.equal(statSync(configPath).mtimeMs, before, 'the file was not rewritten');
  assert.equal(read(configPath).relationshipMemory.capPerDay, 5);
});

test('a zero cap is an owner choosing no cards, and stays', () => {
  // relationshipCap treats anything that is not a positive integer as "no
  // cap configured" and serves nothing. A 0 in the file is therefore the
  // fail-closed state expressed deliberately, and this must not read it as an
  // absence and turn the feature on.
  const configPath = tempConfig({ relationshipMemory: { capPerDay: 0 } });
  assert.deepEqual(ensureRelationshipCap({ capPerDay: 1, configPath }), { capPerDay: 0, changed: false });
  assert.equal(read(configPath).relationshipMemory.capPerDay, 0);
});

test('a cap that is not a small positive integer throws and writes nothing', () => {
  const configPath = tempConfig({ selfName: 'Owner' });
  for (const capPerDay of [0, -1, 1.5, MAX_CAP_PER_DAY + 1, 100, '1', null, NaN]) {
    assert.throws(() => ensureRelationshipCap({ capPerDay, configPath }), /capPerDay must be an integer/u,
      `${JSON.stringify(capPerDay)} must be refused`);
  }
  assert.deepEqual(read(configPath), { selfName: 'Owner' });
  assert.equal(ensureRelationshipCap({ capPerDay: MAX_CAP_PER_DAY, configPath }).changed, true,
...
test('onboarding records the one-a-day cap a fresh install has no config for', async () => {
  await withHome(async ({ home, call }) => {
    ...
    const res = await call('POST', '/admin/config/card', { capPerDay: 1 });
    assert.equal(res.status, 200);
    assert.deepEqual(await res.json(), { state: 'ok', capPerDay: 1, changed: true });
...
test('the card route never overwrites a cap the owner already set', async () => {
  await withHome(async ({ home, call }) => {
    writeFileSync(configPathIn(home), JSON.stringify({
      relationshipMemory: { capPerDay: 4, mode: 'investor' },
    }, null, 2));
    const res = await call('POST', '/admin/config/card', { capPerDay: 1 });
    assert.deepEqual(await res.json(), { state: 'ok', capPerDay: 4, changed: false });
    const written = readConfig(home);
    assert.equal(written.relationshipMemory.capPerDay, 4);
    assert.equal(written.relationshipMemory.mode, 'investor');
  });
});

test('the card route refuses a cap that is not a small positive integer', async () => {
  await withHome(async ({ home, call }) => {
    writeFileSync(configPathIn(home), JSON.stringify({ selfName: 'Owner' }, null, 2));
    for (const capPerDay of [0, -1, 1.5, 11, 500, '1', null]) {
      const res = await call('POST', '/admin/config/card', { capPerDay });
      assert.equal(res.status, 400, `${JSON.stringify(capPerDay)} must be refused`);
    }
    const extra = await call('POST', '/admin/config/card', { capPerDay: 1, engine: 'claude-cli' });
    ...
    const wrongType = await call('POST', '/admin/config/card', { capPerDay: 1 },
