...
// not among them -- so every one of those branches is dead code across nearly
// the whole iMessage corpus, and group messages travel the
...
// one-to-one. Both markers were confirmed on a private development corpus.
//
// A WARNING FOR THE NEXT PERSON, because it cost a full investigation here: the
// service token can be the literal string `any`,
...
//
// THREE-VALUED, NOT BOOLEAN. Some iMessage rows carry no chat_guid at all, and
// there is no way to know what they were. A boolean would silently
...
// moved. It would also split the corpus into new rows that carry the key and
// old rows that do not, which is the exact failure
...
// Checked against private rows that DO carry is_group -- the two agree, so this
// is a fallback and not a second opinion.
...
// deliberately false here: an assertion about a room must be earned, and rows
// with no guid cannot earn it. Callers that instead want "may I credit this as a private
...
// missing on many one-to-one rows the owner sent. Every one of them was dropped
// on the floor, which can remove most of the owner's side of their conversations.
//
// The counterparty was never actually missing: a one-to-one guid is
// `<service>;-;<their handle>`, and the third field IS that handle. Private
// testing found near-total agreement with rows that do carry a handle; the rare
// differences were handle-format drift, not a different person.
//
...
// would mint rooms as people, complete with message counts, and they would be
// indistinguishable from real contacts. Group rows can have no handle and
