...
import { MIN_ROWS, openSummariesDb, SUMMARY_SOURCES } from './summary.mjs';
...
export class PeopleYearCompletion {
    ...
    this.withDb((db) => {
      ...
      try {
        ...
        const insert = db.prepare(
          'INSERT INTO summary_year_people(year, person_key, state) VALUES (?, ?, ?)'
        );
        for (const key of eligible) insert.run(year, key, 'pending');
        db.exec('COMMIT');
      } catch (error) {
        db.exec('ROLLBACK');
        throw error;
  ...
  record({ key, year, result }) {
    ...
    return this.withDb((db) => {
      ...
      db.prepare(
        'UPDATE summary_year_people SET state = ? WHERE year = ? AND person_key = ?'
      ).run(state, year, key);
